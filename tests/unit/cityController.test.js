...
// Mock the fetchDataForNearestCity function
// jest.mock('../../src/clients/iqairClient', () => ({
//     fetchDataForNearestCity: jest.fn(),
// }));
...
describe('Test getNearestCityPollution', () => {
    ...
    it('should return a 400 error for fetchDataForNearestCity error', async () => {
        const req = {
          query: {
            lat: 12.345,
            long: 67.890,
        ...
        jest.mock('../../src/clients/iqairClient', () => ({
            fetchDataForNearestCity: jest.fn().mockRejectedValue(new Error('Fake fetchData error')),
        }));
    
        await getNearestCityPollution(req, res);
    
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({ error: expect.stringMatching(/Error fetching air quality data:/) });
    })
});
